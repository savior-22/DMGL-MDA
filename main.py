...
class GCNnet(torch.nn.Module):
    def __init__(self, in_channelsD, in_channelsM, hidden_channels, out_channels):
        ...
        #self.conv1 = GATConv(hidden_channels, out_channels, heads=2)  #消融TF的时候GAT用这一层，别的不动，注释掉27行，然后下面把73，74行注释掉就行（正常情况用27行这一层GAT）
        ...
        #self.convtf1 = TransformerConv(hidden_channels, hidden_channels, heads=4, dropout=0.1)  #消融GAT的时候用这一层，别的不动，注释掉36行和下面的71行（正常情况用36行这层）
    ...
    def forward(self, xd, xm, edge_index, edge_index_test):
        ...
        #xd = torch.cat([xd,matrix],dim = 1)
        xd = (xd + matrix)/2    #消融双视图的时候注释掉这一行
...
if __name__ == '__main__':
    ...
    np.savetxt(r'D:\code\DMIbidui\Graph2MDA-master\data\data\adjd2', edges)
    ...
    for train_idx, test_idx in stratified_kfold.split(sample_all, label_all):
        ...
        #break
        j = j + 1
        np.savetxt('3/result'+ str(j) +'.txt', result)  #数据保存在3那个文件夹下
